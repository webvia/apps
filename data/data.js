// >> Data ---------------------------------------------------------------------------------------------------------------------------------------------------------------
// $.data.datasets['dataset'].nodesets['nodeset'].nodes['node'].nodes['node']...
...
function SetCont$(key_str){ SetURL$( { action:'history.replaceState', url:href, params:[ { key:'key', value:key_str } ] } );  key_prm=prms.get('key');  

let nks=key_str.split('~~')[0];  let nki=key_str.split('~~')[1];  let nka=nks.split('--');  nka.shift();  let n=nodes;  let current_node={};  

// >> Tree ---------------------------------------------------------------------------------------------------------------------------------------------------------------

let tree_html='';  let tree_item_type='';  
