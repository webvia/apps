...
// GetData -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function GetData$(x){ x=ParseJSON$(x);  let u=x.url; let o=x.object; let p=x.path; let q=x.query;  let i=x.id; let t=x.type; let a=x.action;
/*URL*/ if( HasValue$(u) && !HasValue$(x.data) ){ x.function='SetData$';  Fetch$(x);  return x };  delete x.function;


/*OBJ*/ if( HasValue$(o) ){ let od;  if( !HasValue$(x.data) ){ x.data=ParseJSON$(eval(`${o}`)); od=structuredClone(x.data) };  x.object_data=od };

/*PTH*/ if( HasValue$(p) ){ let pd;  if( HasValue$(x.object_data) ){ pd=structuredClone(x.object_data) }  else{ pd=structuredClone(x.data) };  let pa=p.split('.');  for( let pai of pa ){ pd=pd[pai] };  x.path_data=pd };



/*QRY*/ if( HasValue$(q) && ( HasValue$(x.path_data) || HasValue$(x.data) ) ){ let qd;  if( HasValue$(x.path_data) ){ qd=structuredClone(x.path_data) } else{ qd=structuredClone(x.data) };  let qpka=q.match(/\{\{.+?\}\}/g);  qpka=RemoveArrayDuplicates$(qpka);
  for( let [ik,iv] of Object.entries(qd) ){ let iq=q;
    for( let qpki in qpka ){
      for( let [pk,pv] of Object.entries(iv) ){ let qpk=`\{\{${pk}\}\}`; if(qpka[qpki]===qpk){ iq=iq.replaceAll( qpk, typeof pv==='string'?`'${pv}'`:pv ) } }/*-props*/
      if( Evaluate$(iq)===false ){ delete qd[ik] }
    }/*-keys*/
  }/*-items*/
  x.query_data=qd };
console.log(x);  return x;
}/*-GetData$*/  //let item_prm=prms.get('item');  item_path = (item_prm!=null) ? item_prm : item_path;
